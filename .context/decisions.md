...
> **STATUS: AWAITING HG-10 LAWYER RATIFICATION — DO NOT MERGE UNTIL THE LABOUR-LAWYER HAS RATIFIED THE COPY BELOW.**
...
- **Key count:** 134 user-facing string keys (top-level `onboarding.*` namespace) + 18 screen-reader keys (`a11y.onboarding.*` namespace) + 1 `_meta` metadata block (machine-readable, non-translatable) = **152 catalog entries** across 9 sub-namespaces (`step_indicator`, `advisory_d1`, `browser_baseline_d2`, `passkey_d3`, `passphrase_d4`, `sessions_d5`, `panic_wipe_d6`, `completion_d7`, `show_again`).
...
The `a11y.onboarding.*` namespace contains 18 screen-reader announcement strings covering: wizard step changes (`step_change` with `{n}`/`{m}`/`{step_name}` interpolations per Designer's pass §G advisory); the wizard's landmark label; the recovery-passphrase reveal lifecycle (announcement, in-progress, hidden, capped); modal open/close announcements; the destructive-confirm dialog opening (including the explicit "Escape will not close this dialog — use the Cancel button" notice for keyboard-only users per design-system §3.2 protected-modal pattern); panic-wipe lifecycle announcements (in-progress, complete, partial-failure); session-revocation success; browser-baseline pass/fail; per-step loading and error announcements; the device-fingerprint card's announcement (per M-101c — explicitly tells SR users the fingerprint is rendered client-only and never sent to the server).
