...
> **Inputs consumed:** ADR-0020 in full (Decision 3.e no-PII-in-errors, Decision 7 D.1/D.2/D.4/D.6/panic-wipe WHAT-must-be-communicated lists, Decision 11 closed-allowlist copy-keys.ts contract); Designer's pass (canonical D.1→D.7 labelling including D.5 = session-revocation primer with FIXED labels "Sign out other devices?" / "Revoke other sessions" / "Skip — I'll do this later"; design-system §4 Surface D.T19 8 sub-surfaces); Threat-modeler's §8.T19 F-101 → F-115 (every copy-touching mitigation: M-101a 4 clauses, M-103a TOTP-locked key, M-103c collapsed 410/401 surface, M-104c autocomplete/spellcheck/autocapitalize/autocorrect, M-106a audit-emit-failure user-facing copy, M-108c live-region wording, M-110a closed-allowlist error keys, M-110b Argon2id-canonical-symbol-suppression, M-114c implicit-admin invariant, M-115 panic-wipe modal four-regex contract); design-system §3.4 form-validation copy patterns + §3.5 destructive-confirm canonical pattern; constraints.md (PIPEDA 4.3.4, no-PII-in-errors, AODA WCAG 2.0 AA cognitive-accessibility); existing root catalog `/home/user/agent-os/i18n/en-CA.json` (voice + tone precedent — `onboarding.*`, `panic.*`, `session.*`, `lock.*` keys already shipped at the root level); the 195-line test scaffold at `apps/web/test/T19/onboarding.test.ts` (regex assertions); `RecoveryPassphraseScreen.svelte` (existing surface — keys it already consumes are `onboarding.recovery.heading`, `onboarding.recovery.body`, `onboarding.recovery.show_again.label`, `onboarding.recovery.show_again.helper`, `onboarding.recovery.show_again.helper_capped`, `common.errors.generic`; these live in the ROOT catalog and are NOT redefined in the new T19-scoped catalog — see "Catalog placement note" below).

### Catalog file path + key count

- **File created:** `/home/user/agent-os/apps/web/src/lib/i18n/onboarding.en-CA.json`
- **Locale:** en-CA only (fr-CA deferred to G-T19-1 per Q5).
- **Key count (re-measured 2026-05-25 per G-T19-12):** 151 user-facing string keys (top-level `onboarding.*` namespace) + 26 screen-reader keys (`a11y.onboarding.*` namespace) + 1 `_meta` metadata block (machine-readable, non-translatable) = **178 catalog entries** across 9 sub-namespaces (`step_indicator`, `advisory_d1`, `browser_baseline_d2`, `passkey_d3`, `passphrase_d4`, `sessions_d5`, `panic_wipe_d6`, `completion_d7`, `show_again`). (Prior 2026-05-24 packet recorded 134 + 18 + 1 = 152; the +17 `onboarding.*` / +8 `a11y.onboarding.*` growth is regeneration-noted at the top of this pass and audited in the "Substantive-delta audit note" subsection.)
- **Catalog placement note:** the new T19 catalog is a SEPARATE file scoped to the T19 wizard + panic-wipe surfaces. The existing root catalog at `/home/user/agent-os/i18n/en-CA.json` retains its `onboarding.recovery.*` keys (consumed by the existing `RecoveryPassphraseScreen.svelte` — see hard-coded-strings note below) and its `panic.*` / `session.*` / `lock.*` / `print.recovery.*` keys (used by other surfaces). The T19 implementer's `lib/onboarding/copy-keys.ts` closed-allowlist (per ADR-0020 Decision 11) should reference the new scoped catalog for D.1→D.7 + panic-wipe; the implementer + the new i18n loader resolve from the merged surface of both files. The split is intentional so fr-CA drop-in (G-T19-1) only touches the T19 catalog without re-translating already-shipped strings.

### Hard-coded-strings note (for the implementer)

`apps/web/src/lib/onboarding/recovery/RecoveryPassphraseScreen.svelte` currently calls `t('onboarding.recovery.heading')`, `t('onboarding.recovery.body')`, `t('onboarding.recovery.show_again.label')`, `t('onboarding.recovery.show_again.helper')`, `t('onboarding.recovery.show_again.helper_capped')`, and `t('common.errors.generic')` from the root catalog. These are NOT hard-coded strings — they are catalog-keyed already. **No tech-writer migration required for that file.** The implementer's task #6 is to (a) wire the NEW D.4 wrapper (`D4RecoveryPassphrase.svelte`) to consume keys from `onboarding.passphrase_d4.*` in the new T19 catalog, (b) preserve the existing `RecoveryPassphraseScreen.svelte`'s root-catalog key consumption (the show-again surface inside D.4 keeps its current keys; the surrounding D.4 wrapper consumes the new keys). No string in either file should be hard-coded in JSX/Svelte markup after the implementer's pass.

### HG-10 ratification packet (labour-lawyer-facing summary)

This is the 7-paragraph summary the labour-lawyer reviews before merge. Each paragraph names the surface, the user-facing language we propose, the legal/regulatory rationale, and what the user is consenting to or being warned about. The lawyer's ratification is per-paragraph: any single paragraph the lawyer marks BLOCKER stops the merge.

**Paragraph 1 — D.1 personal-device advisory (PIPEDA 4.3.4 informed-of-purpose + ADR-0008 honest framing).** The first screen the user ever sees says, in plain English: this device will store the sign-in key; do not use a phone or laptop your employer owns, pays for, or manages; the app cannot detect this for you; refreshing the page restarts setup; and a worker without a personal smartphone should talk to their worker co-chair before continuing. The user must explicitly tick a confirmation checkbox AND click "This is a personal device — continue" to advance — no auto-advance, no keyboard skip. The lawyer reviews: (a) is the disclaimer about employer-managed devices accurate and defensible for an Ontario JHSC context, (b) does the "this app cannot detect this for you" framing satisfy ADR-0008's honest-framing posture without creating misleading-statements liability, (c) is the "talk to your worker co-chair" fallback appropriate given the worker may not have any way to contact a co-chair through this app yet.

**Paragraph 2 — D.2 hosting tradeoff (PIPEDA 4.8 openness + cross-border-data disclosure per ADR-0001).** The user is told their committee's data is stored on Supabase, a US-based company that runs servers in Canada (ca-central-1); that worker-side content is encrypted on the device before it leaves; that US legal process could in principle reach the company but what they would get back is the same scrambled bytes; and that the unscrambling key lives only on devices belonging to the user and the committee. A link to the full privacy policy is offered. The reading-level is grade-8 (no Latin abbreviations, no industry jargon). The lawyer reviews: (a) is the "US legal process" framing accurate without overstating residual exposure, (b) is "Supabase" naming the right level of disclosure (vs. just "our hosting provider"), (c) does this paragraph satisfy informed-consent for any cross-border-transfer disclosures Ontario or PIPEDA require at the consent moment.

**Paragraph 3 — D.4 recovery-passphrase purpose statement (PIPEDA 4.3.4 + 4.7 safeguards).** The recovery-passphrase screen states: the passphrase is the only way back into the account if the user loses this device AND their passkey; there is no admin who can reset it; we do not store it and cannot see it; the user should write it down or print the recovery sheet and store it somewhere safe and NOT at work; a passphrase saved only on this device is not a backup; refreshing the page now generates a new passphrase and restarts. A "hold to reveal" affordance shows the passphrase, with a per-setup-session limit of three reveals (M-54c). A downloadable encrypted backup file is offered as an optional secondary copy with clearly-labelled helper text distinguishing the FILE from the PASSPHRASE. The lawyer reviews: (a) is the "no admin can reset it" claim defensible (the architect's recovery-reset path is co-chair-initiated 4-eyes per G-T07-8 — the user-facing copy here says "no admin" because there is genuinely no individual-admin reset; the co-chair-initiated reset is a separate workflow not surfaced at D.4), (b) is "store it somewhere safe and not at work" the right level of operational instruction without crossing into legal advice, (c) does the optional download affordance need additional liability framing about user-side custody.

**Paragraph 4 — D.6 type-back verification + D.6 input-attribute disclosure.** The type-back screen says: type the passphrase exactly as shown; three wrong attempts returns you to D.4 (the passphrase is re-displayed). The input field has `autocomplete="off"`, `spellcheck="false"`, `autocapitalize="none"`, `autocorrect="off"` per M-104c (defense against Chromium cloud-spellcheck round-tripping the passphrase to Google + defense against password-manager autofill polluting the user's vault). No legal disclaimer is surfaced on D.6; the screen is operational only. The lawyer reviews: (a) whether the input-attribute defenses described here require a user-facing disclosure ("we have told your browser not to autocorrect or spellcheck this field") or whether silent technical hardening is acceptable.

**Paragraph 5 — D.6 panic-wipe modal: what it does and what it does NOT do (the M-115 four-regex contract + the residual-risk callout per §8.T19 panic-wipe local-only block).** The modal heading is "Wipe this device's data". The body has FOUR sections the user reads before they can type "WIPE" and click confirm: (a) what happens — the encrypted local store, current sign-in, cached settings, and queued entries on THIS device are removed immediately and cannot be undone on this device; (b) what does NOT happen — this does NOT sign the user out on the server or other devices, the committee can still see prior contributions, and to sign out other devices the user must separately use "Revoke other sessions" in Settings → Sessions; (c) residual-risk callout for coerced users — a session signed in before the wipe can stay valid on the server for up to 15 minutes; as soon as the user safely can, they should sign in on a different personal device and revoke any other sessions; (d) recovery reminder — if the user has their recovery passphrase off this device, they can sign in again on another personal device; if not, they must ask the worker co-chair to send a new invite, and neither the co-chair nor we can recover the old passphrase. The literal phrase "WIPE" must be typed in capital letters. Escape does NOT dismiss; only the explicit Cancel button does. The lawyer reviews: (a) is the "cannot be undone on this device" claim defensible without overstating recoverability of the SERVER-SIDE data (which is intact), (b) does the coerced-user residual-risk callout adequately discharge our duty to inform a user who may be under duress without creating false reassurance, (c) does the "neither the co-chair nor we can recover the old passphrase" claim accurately reflect the architecture (it does, per ADR-0003 Amendment G + ADR-0020 Decision 5), (d) is the 15-minute server-session-validity disclosure accurate enough without leaking enough operational detail to invite session-replay attacks; (e) on an audit-write failure the modal now shows "We couldn't record this wipe before starting it, so we did not wipe anything. Check your internet connection and try again. If you need to leave the app right now, close this tab — nothing on this device has been deleted yet." (the F-106 / M-106a audit-before-side-effect guarantee, surfaced 2026-05-25 per A-T19-RR-1; previously dead code — the modal left the user on a silent in-progress overlay — now user-reachable in a `role="alert"` that destroys nothing, fail-closed); is this safety claim accurate and appropriate to show a potentially-coerced user, and does it risk false reassurance if the user believes a wipe occurred when it did not.

**Paragraph 6 — D.5 session-revocation primer (post-onboarding session-management framing).** The D.5 screen heading is "Sign out other devices?" (FIXED by Designer per ADR-0020 §A — tech-writer does not invent). The body says: if the user has ever signed in on another phone or laptop, they can sign those sessions out now; this is optional and can be done any time later from Settings; sign-outs happen on the server; the committee can still see everything contributed. The primary action is "Revoke other sessions" (destructive button style per §3.5). The tertiary action is "Skip — I'll do this later". An empty-state message handles the "only this device is signed in" case. Helper text and a row-per-other-session list lets the user see what they will sign out (device, OS/browser, last-seen — no IP, no location). The lawyer reviews: (a) is "sign-outs happen on the server" + "the committee can still see everything you've contributed" the right framing to avoid implying a session sign-out also removes committee records, (b) is "Skip — I'll do this later" the right opt-out wording or does it need stronger acknowledgement that the user is choosing not to act.

**Paragraph 7 — D.7 completion + ongoing controls.** The completion screen says "You're set up", shows a four-item checklist (passkey set, recovery sheet printed and confirmed, recovery backup file downloaded if the user chose to, other sessions reviewed), and tells the user where to find panic-wipe (Settings → Wipe this device), session revocation (Settings → Sessions), and a reminder that the recovery passphrase is the only way back if the user loses this device and their passkey. The lawyer reviews: (a) does the completion checklist need explicit attestation language ("by clicking Open the app, you confirm…") or is the per-step explicit-click consent enough, (b) is "Open the app" the right finalizer label vs. something more attestation-shaped.

**Cross-cutting items the lawyer reviews across all 7 paragraphs:** (i) no PII appears in any string per ADR-0020 Decision 3.e + M-110a — every interpolation is non-PII counter ({n}, {m}, {attempts_left}, {other_count}, {failed_systems}); (ii) every error message says what happened, why, and what to do next per the project's tech-writer error-message contract — never "Something went wrong"; (iii) no user-blame phrasing ("you entered an invalid…") — replaced throughout with "that didn't match" / "we couldn't…" patterns; (iv) no marketing voice anywhere in transactional copy; (v) no dark-pattern button styling — "Skip — I'll do this later" is a tertiary affordance equal in visual weight to other tertiary actions per design-system §3.5 (designer-enforced), and the Cancel button in PanicWipeModal is NOT visually de-emphasized vs. the destructive button; (vi) reading level — every paragraph above is at or below grade 9 by Flesch-Kincaid; the D.2 body is grade 8 per the scaffold's no-Latin-abbreviation pin; (vii) `WIPE` is uppercase + language-neutral so the fr-CA drop-in does not require re-translating the type-back gate (G-T19-1 future work).

### Substantive-delta audit note (G-T19-12, 2026-05-25)

Substantive delta folded into Paragraph 5(e); all other +17/+8 keys operational — see regeneration note.

### A11y-string catalog summary

The `a11y.onboarding.*` namespace contains 26 screen-reader announcement strings covering: wizard step changes (`step_change` with `{n}`/`{m}`/`{step_name}` interpolations per Designer's pass §G advisory); the wizard's landmark label; the recovery-passphrase reveal lifecycle (announcement, in-progress, hidden, capped); modal open/close announcements; the destructive-confirm dialog opening (including the explicit "Escape will not close this dialog — use the Cancel button" notice for keyboard-only users per design-system §3.2 protected-modal pattern); panic-wipe lifecycle announcements (in-progress, complete, partial-failure); the panic-wipe type-back aria-label (`panic_wipe_type_back_label`); session-revocation success; browser-baseline pass/fail; per-step loading and error announcements; the device-fingerprint card's announcement (per M-101c — explicitly tells SR users the fingerprint is rendered client-only and never sent to the server); the step-indicator's progress landmark (`step_indicator_landmark`); the three step-pill state labels exposed to SR users — completed, current, and not-yet-reached (`step_pill_completed` / `step_pill_current` / `step_pill_pending`, with `{n}`/`{m}` non-PII counter interpolations); and the browser-baseline failed-checks region — its list label (`failed_checks_list_label`) plus the per-capability failed label (`failed_capability_label`, with a non-PII `{key}` capability-name interpolation) that names each failed browser capability for SR users on the D.2 fail path.

**Live-region usage:** the wizard chrome's `aria-live="polite"` region announces step changes (per design-system §4 Surface D.T19.a). The panic-wipe in-progress overlay's status announcement is `polite` (per design-system §4 Surface G amended row + accessibility-specialist's advisory in ADR-0020 §G). The recovery-passphrase reveal element itself MUST NOT have `aria-live` or `role="alert"` or `role="status"` per F-108 M-108c — the reveal is screen-readable as static `<code>` text only; the live-region announcement is a SEPARATE non-leaking string ("Passphrase is now visible") that does NOT contain the passphrase value. The destructive-confirm dialog opening uses `role="alert"` for the modal heading + an explicit `a11y.onboarding.destructive_confirm_announcement` that primes the user that Escape will not dismiss (consistent with design-system §3.2 protected-modal pattern).

**Reduced-motion considerations:** the catalog itself contains no motion-dependent copy; the implementer's reduced-motion fallback (per Designer's pass §F4 + design-system §4 Surface D.T19.a) replaces motion with static state transitions that consume the same `a11y.onboarding.step_change` announcement. The panic-overlay's reduced-motion spinner-replacement (per design-system §4 Surface G `panic-wipe in progress overlay` row) uses the same `a11y.onboarding.panic_wipe_in_progress_announcement` ("Wiping local data. Do not close this tab.") whether the spinner is animated or static.
...
1. **D.6 input-attribute user-facing disclosure question.** Per M-104c, the D.6 type-back input has `autocomplete="off"` + `spellcheck="false"` + `autocapitalize="none"` + `autocorrect="off"`. The tech-writer's draft does NOT add a user-facing disclosure of this. The lawyer (HG-10 paragraph 4) may decide a one-line disclosure is needed ("Spellcheck and autocorrect are turned off for this field so the passphrase does not leave your device"). If the lawyer requires it, a new key `onboarding.passphrase_d4.confirm_input_disclosure` lands in a tech-writer follow-up pass. **Decision needed from architect:** is this a tech-writer-only iteration, or does it touch the D.6 surface DOM (does the disclosure need to be `aria-describedby`-linked to the input)?

2. **D.4 `download_helper` distinguishes FILE from PASSPHRASE.** The current draft says "This file is the encrypted backup of your account key. It is NOT the passphrase — the passphrase is what unlocks it." This framing is HG-10-paragraph-3 sensitive: a lawyer may find "account key" too technical, or prefer "identity key" / "sign-in key". Tech-writer chose "account key" to match the D.1 advisory's "the key that signs you in" wording for consistency. Architect adjudication is NOT required; flagged for lawyer review only.

3. **`onboarding.sessions_d5.helper_only_this_device` empty-state copy edge case.** The Designer's pass §4 Surface D.T19.g spec says when no other sessions exist, the primer's Skip becomes the only forward action. The tech-writer draft adds the empty-state copy "Only this device is signed in. There is nothing to sign out." This is consistent with the Designer's spec but the architect did NOT explicitly enumerate this string in Decision 7. **Confirming the architect that this addition is in-scope** (it is operational copy needed to make the empty-state coherent; no new affordance is introduced).

4. **Catalog placement decision.** The tech-writer chose to ship the T19 catalog as a NEW scoped file at `apps/web/src/lib/i18n/onboarding.en-CA.json` rather than extending the root `/home/user/agent-os/i18n/en-CA.json` directly. Rationale: (a) keeps T19-scope diff reviewable in isolation; (b) makes the fr-CA drop-in (G-T19-1) a single-file translation pass; (c) the existing root `onboarding.*` keys consumed by `RecoveryPassphraseScreen.svelte` are PRESERVED and not duplicated. **Architect confirmation requested:** the implementer's `lib/onboarding/copy-keys.ts` closed-allowlist + the i18n loader must be wired to read from BOTH catalogs (root + scoped); the existing loader at `apps/web/src/lib/i18n/index.ts` currently reads from the root only. If the architect prefers a single-catalog posture, the tech-writer's draft can be merged into the root file in a follow-up sweep without copy changes.

5. **No flag for `onboarding.panic_wipe_d6.completion_next_steps`.** The completion screen after a successful wipe surfaces three action choices (sign in again with the recovery passphrase; sign in on another device + revoke other sessions; close this tab). This is consistent with M-115 and the §8.T19 residual block. Architect adjudication is NOT required — flagged here for completeness only.

6. **No flag on F-103 / G-T05-11 collapsed user-surface copy.** Per M-103c, the D.3 user-visible error for TOTP `expired` vs `consumed` is collapsed into a single string (`onboarding.passkey_d3.error.enrollment_failed_generic` + the dedicated `error.totp_invalid` + `error.totp_rate_limited` + `error.totp_locked` for the distinguishable TOTP-side cases). The network-observable 410/401 differential remains open per G-T05-11; the tech-writer cannot close it at the copy layer, and the threat-modeler explicitly recorded this as architect-adjudication carry-forward. **No new flag** — recorded for traceability.

### Files modified by this pass

- `/home/user/agent-os/apps/web/src/lib/i18n/onboarding.en-CA.json` — NEW; 152 catalog entries (134 user-facing + 18 a11y + 1 `_meta` block) across the 9 sub-namespaces named in Decision 11.
- `/home/user/agent-os/.context/decisions.md` — this Tech-writer's pass subsection appended to ADR-0020.

### Verdict: READY-FOR-LAWYER

The catalog satisfies every copy-touching mitigation from §8.T19 (F-101 M-101a 4 clauses, F-103 M-103a `totp_locked` + M-103c collapsed user-visible 410/401 surface, F-106 M-106a `audit_emit_failed` toast copy, F-108 M-108c live-region wording, F-110 M-110a/b argon2-canonical-symbol-suppression, F-115 M-115 four-regex modal copy). Every scaffold regex assertion (`/personal device/i`, `/personal device.*continue/i`, `/stop.*switch.*personal/i`, `/encrypted|scrambled/i`, `/Canada|ca-central|Canadian/i`, no `\bi\.e\.|\be\.g\.`, `/browser is too old/i`, `/set up passkey/i`, `/wipe/i`, the F-115 four-regex set) is satisfied by the corresponding catalog values. Six items are flagged for architect attention above (none are blockers — three are confirmations-requested, one is a lawyer-iteration question, two are tech-writer's-own-rationale recorded for traceability). **The labour-lawyer's HG-10 ratification of the 7-paragraph packet above is the only blocking gate before T19 implementer task #6 can merge.**
...
- [x] **threat-modeler next pass — RATIFIED 2026-05-23.** `.context/threat-model.md` §3.10 "Backup posture (T17)" assigns final F-### range **F-70..F-85** (NO renumbering — placeholders were sequential after §3.9's F-55..F-69 with no collision). All sixteen findings scored Low residual at the library boundary; eight carry `[T17.1 deferred]` tags (F-70 cross-mirror SQL drift, F-71 bucket policy, F-72 single-tx wrapper, F-74 lifecycle policy backstop, F-75 A-BACKUP-001 alert sink, F-76 SQL `xact_start()` ordering, F-79+F-80 pgTAP column assertions, F-83 T18 integrity-job consumer). All eight architect asks validated (no BLOCK): (1) F-72 is coherent F-24 variant — state-machine shape; (2) F-71 is NEW SURFACE — no equivalent in §3.1–§3.9; (3) F-83 scored as load-bearing with snapshot-pinned field names; (4) RA-2 control #4 STRENGTHENED (witness now has structured testimony format); (5) RA-2 trigger #3 NOT re-opened (walked all four triggers); (6) HG-10 NOT firing defensible (PIPEDA Principle 4.8 preserved via policy + audit-log paths); (7) cross-border egress invariant preserved (`cross_region_destination_refused` structured rejection); (8) no new subprocessor (ADR-0010 preserved). New carry-forwards: G-T17-1..G-T17-10 + G-T17-RA2-ANCHOR-CONSUMER + G-T17-T16-NO-COUPLING + G-T17-BUCKET-POLICY-RATIFY + G-T17-RESTORE-DRILL-CADENCE. Original placeholder ask retained below for traceability:
