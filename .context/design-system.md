...
- **Plain spaced letters, not NATO phonetic (recommended, flagged).** The `aria-label` spells chars as plain space-separated glyphs (`"c 3 d 4"`) — TTS pronounces hex letter names clearly and this matches the app's established digit-by-digit pattern. NATO phonetic (`"charlie three delta four"`) is **not** baked in by default: it ~roughly doubles announcement length and forces the co-chair's side to decode phonetics too. **However**, the hex letters `a–f` are genuinely confusable over a noisy voice channel (`b`/`d`/`e`/`p`, `a`/`8`, `c`/`e`, `f`/`s`), and this is the one channel where a single confusion defeats F-172 — so **this is the single most important open question for the accessibility-specialist** (see Open questions). The group-numbering already localizes any single-char mismatch to one group, which the two humans can then spell out ad hoc.
...
1. **The exact SR announcement format for the 64-hex fingerprint — the single load-bearing decision (F-172).** This pass recommends **grouped + numbered + plain-spelled**: `<ol>` of 16 `<li>`, each `role="img"` with `aria-label` = `"group N of 16, c 3 d 4"` (plain space-separated glyphs, not NATO phonetic). Two sub-decisions the accessibility-specialist should confirm via real SR testing (NVDA + VoiceOver + TalkBack, since the audience is Android-heavy): **(a)** keep the `"group N of 16"` prefix in every group label (recommended — position tracking is the whole game for an out-of-band compare; verbosity is worth it) or drop it and rely on `<ol>`/`<li>` "N of 16" (leaner, but not announced in continuous-read mode); **(b)** whether the `a–f` voice-confusability warrants **NATO phonetic** in the `aria-label` (recommended: **no** by default — keep parity with the app's digit-by-digit pattern and halve the announcement length — but this is the one place a single confusion defeats F-172, so it is explicitly the accessibility-specialist's call, and if adopted it is a copy/label change only, no new token). Whichever is chosen, the co-chair's P1-8d screen (Surface K screen 3) SHOULD mirror it.
