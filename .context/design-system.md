...
- **List vs. table semantics.** The roster ships as **list semantics** (`<ul>`/`<li>`), mirroring `/concerns` and `/reprisal` for cross-surface consistency and mobile-first stacked rows — the roster is read-only in P1-8b (no cell-by-cell comparison need). Each `<li>` is `role="group"` with `aria-labelledby` pointing to its name element, so a screen reader announces each member as a self-contained unit (name → badge → roles → contact → date, in DOM order). The `<ul>` carries `aria-label="Committee members"` (`committee.roster.list_aria`). **If** a future desktop-columned variant is wanted (Surface H/I precedent), it MUST use a real `<table>` with `<caption>`, `<th scope="col">` per column, and `role` mapping — but that is out of P1-8b scope; the stacked list is the P1-8b deliverable.
