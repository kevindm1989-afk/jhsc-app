...
**Status (closed — fully now):** the formal `has_column` / `columns_are` pin residual landed. New pgTAP file `supabase/test/t17_backup_manifests_f83_column_pin.sql` ships 16 assertions: (1) `columns_are` on the full 19-column set — any addition/rename/removal trips it; (2-8) `col_type_is` on the 7 F-83 anchor columns (`audit_log_head_id` bigint, `audit_log_head_ts_ms` bigint, `audit_log_head_hash` bytea, `per_event_row_counts` jsonb, `retention_sweep_runs_snapshot_ts_ms` bigint, `schedule_hash` text, `node_runtime_pin` text); (9-16) `col_type_is` on the surrounding structural fields (`run_id` uuid PK, `manifest_status` text state-machine, `blob_sha256` text + the existing `^[0-9a-f]{64}$` CHECK, `blob_bytes` bigint, etc.). Wired into the CI `Committee DB tests (pgTAP)` job at `.github/workflows/ci.yml`. The implicit-via-INSERTs pin from the prior partial-close remains as belt-and-braces (a column rename now fails BOTH the new explicit assertion and the existing INSERTs).
**Blocker for:** closed.
