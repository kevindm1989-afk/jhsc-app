<script>
  ...
  // surfaces agree.
  const currentMonthString = toMonthString(new Date());
  const currentMonth = buildMonthlyReport(currentMonthString);
  const currentMonthActivity = Object.values(currentMonth.totals).reduce((acc, n) => acc + n, 0);

  // Same calculation for the same month one year earlier so the tile
  // can render a YoY delta indicator matching /report's tile pattern.
