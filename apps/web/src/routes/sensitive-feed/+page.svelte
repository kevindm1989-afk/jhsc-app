<script>
  ...
  $: activeFilters = (() => {
    ...
    if (activeValue) {
      list.push({
        key: 'filter',
        label: `${t('common.activeFilters.axis.sensitivity')}: ${t(`sensitiveFeed.viewer.chip.${activeValue}`)}`,
        removeHref: buildHref('/sensitive-feed', { sort: sortParam, from: fromParam, to: toParam })
      });
    }
    if (fromParam || toParam) {
      list.push({
        ...
        removeHref: buildHref('/sensitive-feed', { filter: filterParam, sort: sortParam })
      });
    }
    if (sortParam === 'oldest') {
      list.push({
        ...
        removeHref: buildHref('/sensitive-feed', {
          filter: filterParam,
          from: fromParam,
          to: toParam
        })
  ...
  function buildDownload() {
    ...
    return {
      csv: withMetadata(
        { route: '/sensitive-feed', filters: activeFilters.map((f) => f.label).join(' · ') },
