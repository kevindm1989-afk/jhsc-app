...
>
  <header class="min-header">
    <h1 id="min-heading">
      {t('minutes.viewer.heading')}{#if filterLabel}<span
  ...
  {#if loading}
    <div role="status" aria-label={t('minutes.viewer.loading')} data-testid="min-loading">
      <SkeletonRows />
    </div>
  {:else if loadError}
  ...
  {:else if rows.length === 0}
    <div class="empty-state" data-testid="min-empty" role="status">
      <p class="muted">
      ...
      {#if filterActive && clearHref}
        <p class="empty-state-actions">
          <a href={clearHref} class="empty-state-clear" data-testid="min-empty-clear">
